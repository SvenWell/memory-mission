...
constraints:
  ...
  - "every IC decision is a Proposal containing a tier=decision page + an UpdateFact (lifecycle_status: ic → decision) + an `ic_decision` predicate fact + (if invest) `valuation_at_entry` + `closing_date` + `lead_negotiator` facts"
